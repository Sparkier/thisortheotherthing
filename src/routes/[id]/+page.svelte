...
<div class="min-h-screen bg-gray-50 flex flex-col items-center py-12 px-4">
	<div class="max-w-2xl w-full bg-white rounded-xl shadow-lg p-8">
		...
		<div class="mb-8">
			<a href="/" class="text-blue-600 hover:underline text-sm font-medium">&larr; Create new poll</a>
		</div>
		...
		{#if data.hasVoted && data.results}
			...
			<div class="space-y-6">
				...
				<div>
					...
					<div class="w-full bg-gray-200 rounded-full h-4">
						<div class="bg-blue-600 h-4 rounded-full transition-all duration-500" style="width: {percentA}%"></div>
					</div>
				</div>
				...
				<div>
					...
					<div class="w-full bg-gray-200 rounded-full h-4">
						<div class="bg-green-600 h-4 rounded-full transition-all duration-500" style="width: {percentB}%"></div>
					</div>
				</div>
			</div>
			...
			<div class="mt-8 pt-6 border-t border-gray-100 text-center">
				...
				<input type="text" readonly value="{data.url}" class="w-full bg-gray-50 border border-gray-200 text-gray-600 text-sm rounded-md px-3 py-2 focus:outline-none focus:ring-1 focus:ring-blue-500" onclick={(e) => e.currentTarget.select()} />
			</div>
