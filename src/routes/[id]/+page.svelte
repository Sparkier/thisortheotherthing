...
<div class="min-h-screen bg-[#FDFBF7] flex flex-col items-center py-16 px-4 font-sans text-gray-800 relative overflow-hidden">
	...
	<div class="fixed top-[-20%] left-[-10%] w-[60%] h-[60%] bg-rose-200/40 rounded-full blur-3xl opacity-50 pointer-events-none"></div>
	<div class="fixed bottom-[-10%] right-[-10%] w-[50%] h-[50%] bg-indigo-200/40 rounded-full blur-3xl opacity-50 pointer-events-none"></div>

	<div class="max-w-3xl w-full bg-white/80 backdrop-blur-xl rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 p-8 md:p-12 relative z-10">

		<div class="mb-10">
			<a href="/" class="inline-flex items-center text-sm font-medium text-gray-400 hover:text-gray-900 transition-colors">
		...
		{#if data.hasVoted && data.results}
			...
			<div class="text-center mb-12">
				<h1 class="text-4xl md:text-5xl font-serif font-medium text-gray-900 mb-4 tracking-tight">The results are in</h1>
				<p class="text-gray-500 font-medium">{totalVotes} {totalVotes === 1 ? 'person has' : 'people have'} voted</p>
			</div>
			...
			<div class="mt-16 pt-8 border-t border-gray-100/60 max-w-md mx-auto text-center">
				...
				<input type="text" readonly value="{data.url}" class="w-full bg-gray-50 border border-gray-200 text-gray-600 text-center text-sm rounded-xl px-4 py-3 focus:outline-none focus:border-gray-300 focus:ring-4 focus:ring-gray-100 transition-all cursor-copy" onclick={(e) => e.currentTarget.select()} />
			</div>

		{:else}
			<!-- Voting View -->
			<div class="text-center mb-12">
				<h1 class="text-4xl md:text-5xl font-serif font-medium text-gray-900 mb-4 tracking-tight">Cast your vote</h1>
				<p class="text-gray-500 text-lg">Which one do you prefer?</p>
			</div>

			{#if form?.error}
				<div class="mb-8 p-4 text-sm font-medium text-red-600 bg-red-50 rounded-2xl text-center border border-red-100">
					{form.error}
				</div>
			{/if}
